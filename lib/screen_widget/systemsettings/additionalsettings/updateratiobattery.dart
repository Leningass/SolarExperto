...
class UpdateBatteryRatio extends StatefulWidget {
  const UpdateBatteryRatio({Key? key, required this.batteryIconratioModel})
      : super(key: key);
  final BatteryIconratioModel batteryIconratioModel;
  @override
  State<UpdateBatteryRatio> createState() => _UpdateBatteryRatioState();
}

class _UpdateBatteryRatioState extends State<UpdateBatteryRatio> {
  ...
  @override
  void initState() {
    numberseries.text = widget.batteryIconratioModel.numberseries.toString();
    numberparallel.text =
        widget.batteryIconratioModel.numberparallel.toString();
    ImageUrl = widget.batteryIconratioModel.icon;
    super.initState();
  }
                    ...
                    ElevatedButton.icon(
                      ...
                      onPressed: () async {
                        if (_batteryiconform.currentState!.validate()) {
                          ...
                          if (file != null) {
                            await uploadImage(
                                    widget.batteryIconratioModel.id, file!)
                                .then((value) => setState(() {
                                      if (value != null) {
                                        ImageUrl = value;
                                      } else {
                          ...
                          await DatabaseService().updatebatteryratio(
                              widget.batteryIconratioModel.id,
                              total,
